...
La respuesta incluye el texto que se necesita para armar el  código QR (en caso que generes el PDF desde tu lado) y/o el viejo código de barras (para comprobantes anteriores). Te sugerimos descargar el pdf y almacenarlo en tu plataforma.
...
### ¿Cómo saber si debo emitir un comprobante de tipo "MiPyme"?

Hay ciertos casos donde AFIP exige que en lugar de emitir una factura A,B o C, debes emitirle a tu cliente FACTURA DE CREDITO ELECTRONICA MiPyME (FCE) A, B o C. En ese caso, recibirás el campo  requiere\_fec = "SI". Ésto se debe a que el emisor o el receptor están informados como Pyme en AFIP y/o el monto del comprobante a emitir supera el limite que AFIP tiene definido. Ésta info varia día a día.  Conocé más [desde aquí](https://developers.tusfacturas.app/api-factura-electronica-afip-facturacion-nuevo-comprobante/api-factura-electronica-afip-factura-de-credito-electronica-mipyme-fce). Te sugerimos comentarlo con tu cliente y asesorarte con su estudio impositivo al respecto.
...
* El número de CAE es un texto y se envía con un espacio al final.
* El texto que se retorna en el campo afip\_codigo\_barras y afip\_qr, se envía con un espacio al final.
* Siempre redondea los valores númericos con "Round Half even".
...
Ten en cuenta que ésto solo está permitido para comprobantes hasta ciertos          montos. Consultá el monto actualizado, con éste método: [Consultá los topes de venta a CF](https://developers.tusfacturas.app/api-factura-electronica-afip-or-consulta-de-tope-para-ventas-a-consumidor-final)
{% endhint %}
